defmodule MvpApi.KnowledgeTest do
  ...
  describe "innovation_projects" do
    ...
    @valid_attrs %{budget: "120.5", expected_anual_effect: "120.5", expected_result: "some expected_result", name: "some name", objective: "some objective", observation: "some observation", term: "120.5"}
    @update_attrs %{budget: "456.7", expected_anual_effect: "456.7", expected_result: "some updated expected_result", name: "some updated name", objective: "some updated objective", observation: "some updated observation", term: "456.7"}
    @invalid_attrs %{budget: nil, expected_anual_effect: nil, expected_result: nil, name: nil, objective: nil, observation: nil, term: nil}
    ...
    test "create_innovation_project/1 with valid data creates a innovation_project" do
      assert {:ok, %InnovationProject{} = innovation_project} = Knowledge.create_innovation_project(@valid_attrs)
      assert innovation_project.budget == Decimal.new("120.5")
      assert innovation_project.expected_anual_effect == Decimal.new("120.5")
    ...
    test "update_innovation_project/2 with valid data updates the innovation_project" do
      innovation_project = innovation_project_fixture()
      assert {:ok, %InnovationProject{} = innovation_project} = Knowledge.update_innovation_project(innovation_project, @update_attrs)
      assert innovation_project.budget == Decimal.new("456.7")
      assert innovation_project.expected_anual_effect == Decimal.new("456.7")
    ...
    test "update_innovation_project/2 with invalid data returns error changeset" do
      innovation_project = innovation_project_fixture()
      assert {:error, %Ecto.Changeset{}} = Knowledge.update_innovation_project(innovation_project, @invalid_attrs)
    ...
    test "delete_innovation_project/1 deletes the innovation_project" do
      ...
      assert_raise Ecto.NoResultsError, fn -> Knowledge.get_innovation_project!(innovation_project.id) end
    end
